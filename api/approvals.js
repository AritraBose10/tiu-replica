...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (req.method === 'POST') {
            const v = validate(schemas.approvals.create, req.body);
            if (!v.ok) return res.status(400).json({ error: 'Validation failed', details: v.errors });
            const { name, full_name, logo, description, sort_order } = v.data;
            await db.execute({
                sql: 'INSERT INTO approvals (name, full_name, logo, description, sort_order) VALUES (?, ?, ?, ?, ?)',
                args: [name, full_name, logo, description, sort_order],
            });
            return res.status(201).json({ success: true });
        }

        if (req.method === 'PUT') {
            const v = validate(schemas.approvals.update, req.body);
            if (!v.ok) return res.status(400).json({ error: 'Validation failed', details: v.errors });
            const { id, name, full_name, logo, description, sort_order } = v.data;
            await db.execute({
                sql: 'UPDATE approvals SET name=?, full_name=?, logo=?, description=?, sort_order=? WHERE id=?',
                args: [name, full_name, logo, description, sort_order, id],
            });
            return res.status(200).json({ success: true });
        }

        if (req.method === 'DELETE') {
            const v = validate(schemas.approvals.delete, req.body);
            if (!v.ok) return res.status(400).json({ error: 'Validation failed', details: v.errors });
            await db.execute({ sql: 'DELETE FROM approvals WHERE id = ?', args: [v.data.id] });
            return res.status(200).json({ success: true });
        }
